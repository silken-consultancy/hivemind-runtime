...
**INV-5 (enforced aqui):** `fos_recall({ mode: "semantic" })` (busca cosine) **NUNCA**
...
<!-- SEGURANÇA (medido — F0/VALIDAÇÃO do plano; owner-scoping por plane, RLS real).
     ...
     SEGURO — toda leitura de memory_index é WHERE owner_id = <owner>, sem exceção por
     ...
     fos_recall({mode:"semantic"}) / cosine — isso violaria INV-5. Planes vazios não
...
- **`self/core`** — a espinha da self-layer: identidade · posture · resonance · purpose ·
  ...
  exato — **nunca** por cosine (INV-5). É quem você é nesta sessão.
...
INV-5-safe (vem no skeleton, não é cosine). Para o corpo completo de uma memória:
`fos_recall({ mode:"exact", name:"..." })` JIT. `kind` é proxy de `memory_index.type`
...
- **Porta 3 — RESSONÂNCIA.** Mid-session, **não** no boot. Os gatilhos vivem em
  `self/core/resonance-how-i-remember`; cosine sobre `self/lived` jamais no boot (INV-5).

Determinístico: `recent_self` por `created_at DESC`, âncoras por topic exato — **não** cosine.
INV-5 intacto.
...
- **Determinístico:** endpoint estruturado com parâmetros fixos. **NÃO** usa cosine, **NÃO**
  viola INV-5.
...
- **Planes vazios são normais:** `os-kernel/*` (e potencialmente `tenant/*`) com `count:0`
  para um owner que nunca os teve semeados é o comportamento CORRETO (owner-scoping + RLS) —
  não lança erro, não dispara fallback, **nunca** substituir por cosine (ver comentário de
...
- **Porta 2 (`recent_self`, passo 1c) é julgamento, não bulk:** títulos garantidos pelo
  ...
  usa cosine (INV-5, Porta 3 é mid-session).
